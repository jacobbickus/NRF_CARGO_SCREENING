        ...
        : DetectorA(DetectorAction)
{
  ...
  CmdAttenOn3->SetGuidance("Choose if Attenuator Third Layer Present or not");
  CmdAttenThick3->SetGuidance("Choose Desired attenuator thickness");
  CmdAttenMat3->SetGuidance("Choose third attenuator layer material");
  ...
  CmdAttenOn2->SetCandidates("On on Off off");
  CmdAttenOn3->SetCandidates("On on Off off");
  ...
  CmdAttenMat3->SetCandidates("G4_Pb G4_Cu G4_Zn G4_Ag G4_Cd G4_Th G4_U G4_Au G4_W G4_Fe G4_POLYETHYLENE G4_POLYPROPYLENE G4_POLYSTYRENE G4_POLYVINYL_CHLORIDE G4_POLYCARBONATE");
...
{ // this function only works if named SetNewValue
  ...
  else if(command == CmdnPMT)
  {
    ...
    G4cout << "The number of photocathodes per cherenkov Detector manually set to: "
            << thecmdnPMT << G4endl;
  }
  else if(command == CmdAngle)
  {
    ...
    G4cout << "The Detector angle manually set to: "
            << thecmdAngle << " degrees" << G4endl;
  }
  else if(command == CmdAttenOn)
  {
    ...
    G4cout << "The first attenuation layer manually set to: "
            << theCmdAttenOn << G4endl;
  }
  else if(command == CmdAttenThick)
  {
    if(check_atten_on)
    {
      ...
      G4cout << "The first attenuation layer thickness manually set to: "
              << theCmdAttendThickness << " cm" << G4endl;
    }
  }
  else if(command == CmdAttenMat)
  {
    if(check_atten_on)
    {
      ...
      G4cout << "The first attenuation layer material manually set to: "
              << theAttenMaterial << G4endl;
    }
  }
  ...
  else if(command == CmdAttenThick2)
  {
    if(check_atten2_on)
    {
      ...
      G4cout << "The second attenuation layer thickness manually set to: "
              << theCmdAttendThickness2 << " cm" << G4endl;
    }
  }
  else if(command == CmdAttenMat2)
  {
    if(check_atten2_on)
    {
      ...
      G4cout << "The second attenuation layer material manually set to: "
              << theAttenMaterial2 << G4endl;
    }
  }
  else if(command == CmdAttenOn3)
  {
    G4String theCmdAttenOn3 = newValue;
    if(theCmdAttenOn3 == "Off" || theCmdAttenOn3 == "off")
    {
      DetectorA->SetAttenuatorState3(false);
      DetectorA->SetAttenuatorMaterial3("G4_AIR");
      check_atten3_on = false;
    }
    else
    {
      check_atten3_on = true;
      DetectorA->SetAttenuatorState3(true);
    }
    G4cout << "The third attenuation layer manually set to: "
            << theCmdAttenOn3 << G4endl;
  }
  else if(command == CmdAttenThick3)
  {
    if(check_atten2_on)
    {
      G4double theCmdAttendThickness3 = CmdAttenThick3->GetNewDoubleValue(newValue);
      DetectorA->SetAttenuatorThickness3(theCmdAttendThickness3);
      G4cout << "The third attenuation layer thickness manually set to: "
              << theCmdAttendThickness3 << " cm" << G4endl;
    }
  }
  else if(command == CmdAttenMat3)
  {
    if(check_atten3_on)
    {
      G4String theAttenMaterial3 = newValue;
      DetectorA->SetAttenuatorMaterial3(theAttenMaterial3);
      G4cout << "The third attenuation layer material manually set to: "
              << theAttenMaterial3 << G4endl;
    }
  }
  else if(command == CmdPlexi)
  {
    ...
    G4cout << "The Plexiglass thickness manually set to: "
            << plexiThickness << " cm" << G4endl;
  }
  else if(command == CmdTape)
  {
    G4double tapeThickness = CmdTape->GetNewDoubleValue(newValue);
    DetectorA->SetTapeThickness(tapeThickness);
    G4cout << "The tape thickness manually set to: "
            << tapeThickness << " mm" << G4endl;
  }
  else if(command == CmdVis)
  {
    ...
    if(theCmdVis == "True" || theCmdVis == "true")
    {
      ...
      G4cout << "Detector Visualization View Only set to True!"
              << G4endl;
    }
  }
