...
G4VPhysicalVolume* ChopperSetup::Construct(G4LogicalVolume* logicWorld, bool checkOverlaps)
{
  ...
  G4cout << "----------------------------------------------------------------------" << G4endl;
  G4double chopper_beginning_edge_position = (source_z_pos*cm + chopper_z) + chopper_thick;
  ...
  G4double begin_from_source = chopper_beginning_edge_position/(cm) - source_z_pos;
  ...
  G4VPhysicalVolume* chopper = new G4PVPlacement(0, G4ThreeVector(0, -2*cm, chopper_center_position),
