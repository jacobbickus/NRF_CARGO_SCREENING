...
G4VPhysicalVolume* ChopperSetup::Construct(G4LogicalVolume* logicWorld, bool checkOverlaps)
{
  ...
  G4cout << "----------------------------------------------------------------------" << G4endl;
  G4double chopper_beginning_edge_position = (source_z_pos*cm + chopper_z) + chopper_thick/4.;
  G4double chopper_end_edge_position = (source_z_pos*cm + chopper_z) + chopper_thick;
